class ValidatorDutiesTracker {
    constructor() {
        ...
        this.beaconUrl = localStorage.getItem('beaconUrl');
        if (!this.beaconUrl) {
            ...
            localStorage.setItem('beaconUrl', this.beaconUrl);
        }
    ...
    async initializeNotifications() {
        ...
        const telegramEnabled = localStorage.getItem('telegramEnabled') === 'true';
        
        if (savedChatId) {
            document.getElementById('telegramChatId').value = savedChatId;
            
            if (telegramEnabled) {
                this.showNotificationStatus('telegram', 'Telegram notifications active', true);
    ...
    async updateTelegramSubscriptionSilent(newValidator = null) {
        ...
        try {
            ...
            if (newValidator) {
                // Get previous validators from local storage
                const previousValidators = JSON.parse(localStorage.getItem('telegramValidators') || '[]');
                ...
                localStorage.setItem('telegramValidators', JSON.stringify(this.validators.map(v => v.id || v)));
            } else {
                // Full update
                await fetch(`${this.serverUrl}/api/telegram/update`, {
                    method: 'POST',
    ...
    loadValidators() {
        const stored = localStorage.getItem('validators');
    ...
    saveValidators() {
        localStorage.setItem('validators', JSON.stringify(this.validators));
    }
    
    loadBlockDetails() {
        const stored = localStorage.getItem('blockDetails');
        if (stored) {
            try {
    ...
    loadMissedAttestations() {
        const stored = localStorage.getItem('missedAttestations');
        if (stored) {
            try {
    ...
    saveMissedAttestations() {
        localStorage.setItem('missedAttestations', JSON.stringify(this.missedAttestations));
    }
    
    clearBlockDetails() {
        this.blockDetails = {};
        localStorage.removeItem('blockDetails');
        this.displayProposerDuties();
    }
    ...
    loadNotifiedDuties() {
        const stored = localStorage.getItem('notifiedDuties');
        if (stored) {
            try {
    ...
    saveNotifiedDuties() {
        localStorage.setItem('notifiedDuties', JSON.stringify([...this.notifiedDuties]));
    }
    ...
    getValidatorCustomLabel(validator) {
        const labels = JSON.parse(localStorage.getItem('validatorLabels') || '{}');
    ...
    setValidatorCustomLabel(validator, label) {
        ...
        localStorage.setItem('validatorLabels', JSON.stringify(labels));
    }
    ...
    loadNotificationSettings() {
        // Load saved settings
        const settings = {
            notifyProposer: localStorage.getItem('notifyProposer') !== 'false',
            notifyAttester: localStorage.getItem('notifyAttester') !== 'false',
            notifySync: localStorage.getItem('notifySync') !== 'false',
            notifyMissed: localStorage.getItem('notifyMissed') === 'true', // Default to false
            notifyMinutes: localStorage.getItem('notifyMinutes') || '10',
            desktopSound: localStorage.getItem('desktopSound') !== 'false',
            desktopPersistent: localStorage.getItem('desktopPersistent') !== 'false'
        };
    ...
    cacheDuties() {
        ...
        localStorage.setItem('dutiesCache', JSON.stringify(cache));
    }

    loadCachedDuties() {
        const cached = localStorage.getItem('dutiesCache');
        if (!cached) return;
    ...
    clearCache() {
        localStorage.removeItem('dutiesCache');
        this.duties = {
            proposer: [],
            attester: [],
            sync: []
        ...
        this.missedAttestations = {};
        localStorage.removeItem('missedAttestations');
    ...
    clearAllData() {
        if (!confirm('Are you sure you want to clear all data? This will remove all validators, settings, and preferences.')) {
            return;
        }
        ...
        // Reset form fields
        document.getElementById('beaconUrl').value = '';
        document.getElementById('publicBeaconSelect').value = '';
        document.getElementById('telegramChatId').value = '';
        document.getElementById('autoRefresh').checked = false;
        
        // Reset notification checkboxes to defaults
        document.getElementById('notifyProposer').checked = true;
        document.getElementById('notifyAttester').checked = true;
        document.getElementById('notifySync').checked = true;
        document.getElementById('notifyMinutes').value = '10';
        document.getElementById('notifyProposerTelegram').checked = true;
        document.getElementById('notifyAttesterTelegram').checked = true;
        document.getElementById('notifySyncTelegram').checked = true;
        document.getElementById('notifyMinutesTelegram').value = '10';
        
        // Hide notification status messages
        document.getElementById('telegramNotificationStatus').textContent = '';
        document.getElementById('browserNotificationStatus').textContent = '';
        
        // Reset button visibility
        document.getElementById('enableTelegramNotifications').style.display = 'inline-block';
        document.getElementById('updateTelegramSubscription').style.display = 'none';
        document.getElementById('testBrowserNotification').style.display = 'none';
        
        // Set a default beacon URL
        const randomIndex = Math.floor(Math.random() * this.publicBeaconUrls.length);
        this.beaconUrl = this.publicBeaconUrls[randomIndex].url;
        localStorage.setItem('beaconUrl', this.beaconUrl);
        document.getElementById('beaconUrl').value = this.beaconUrl;
        document.getElementById('publicBeaconSelect').value = this.beaconUrl;
                ...
                if (data.settings) {
                    try {
                        ...
                        if (data.settings.beaconUrl) {
                            ...
                            localStorage.setItem('beaconUrl', data.settings.beaconUrl);
                        }
                        ...
                        if (data.settings.telegram) {
                            if (data.settings.telegram.chatId) {
                                localStorage.setItem('telegramChatId', data.settings.telegram.chatId);
                                document.getElementById('telegramChatId').value = data.settings.telegram.chatId;
                            }
                            localStorage.setItem('telegramEnabled', data.settings.telegram.enabled ? 'true' : 'false');
                        }
                        
                        // Restore browser notification setting
                        if (data.settings.browser) {
                            localStorage.setItem('browserNotifications', data.settings.browser.enabled ? 'true' : 'false');
                        }
